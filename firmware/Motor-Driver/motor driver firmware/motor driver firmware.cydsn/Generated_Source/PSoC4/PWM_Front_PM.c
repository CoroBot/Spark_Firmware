...
static PWM_Front_BACKUP_STRUCT PWM_Front_backup;
...
{
    if(0u != (PWM_Front_BLOCK_CONTROL_REG & PWM_Front_MASK))
    {
        PWM_Front_backup.enableState = 1u;
    }
    else
    {
        PWM_Front_backup.enableState = 0u;
    }
...
void PWM_Front_Wakeup(void)
{
    ...
    if(0u != PWM_Front_backup.enableState)
    {
        PWM_Front_Enable();
    }
}
